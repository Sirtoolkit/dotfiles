function purge-config
	echo -e "\n\033[1;31mADVERTENCIA:\033[0m Esta acción eliminará completamente la carpeta ~/.config, así como los archivos ~/.bashrc, ~/.tmux y ~/.tmux.conf."
	...
	if test "$CONFIRM" = "si"
		...
		rm -rf ~/.config ~/.bashrc ~/.tmux ~/.tmux.conf
